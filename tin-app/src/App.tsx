...
export default function App(){
 ...
 async function importPhoto(file:File){try{const p=await preparePhoto(file);try{await savePhoto(p);}catch{announce('照片暂时无法保存到本机，离开前请保存 PNG。');}const map=new Map(photosRef.current);map.set(p.id,{url:URL.createObjectURL(p.blob),record:p});photosRef.current=map;setPhotos(map);return p.id;}catch(e){announce((e as Error).message);return undefined;}}
